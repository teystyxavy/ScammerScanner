  ...
  const handleProfileSave = async (e) => {
    ...
    try {
      const response = await fetch(`/api/user/${user.user_id}`, {
        ...
        body: JSON.stringify({ username, email, currentPassword: profileCurrentPassword }),
      });
      ...
      if (response.ok) {
        ...
        setProfileSuccess("");  
        setProfileError(errorData.error || "Failed to update profile. Please try again.");
      }
    } catch (err) {
      setProfileSuccess("");
    ...
    <div className="flex flex-col justify-center items-center mt-16 mb-8">
      <div className="w-full max-w-md mb-8">
        ...
        >
          ...
          {profileError && <p className="text-red-500 text-xs italic mb-4">{profileError}</p>}
          {profileSuccess && <p className="text-green-500 text-xs italic mb-4">{profileSuccess}</p>}
      ...
      <div className="w-full max-w-md">
        ...
        >
          ...
          {passwordError && <p className="text-red-500 text-xs italic mb-4">{passwordError}</p>}
          {passwordSuccess && <p className="text-green-500 text-xs italic mb-4">{passwordSuccess}</p>}
          <div className="flex items-center justify-center mt-6">
            <button
              type="submit"
              className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
