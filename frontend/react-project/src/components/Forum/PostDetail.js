...
function PostDetail({ post, onClose }) {
	...
	useEffect(() => {
		const fetchLikesData = async () => {
			try {
				const response = await fetch(
					`http://localhost:5000/api/posts/${postDetails.post_id}/likes`
				);
				if (response.ok) {
		...
		const fetchComments = async () => {
			try {
				const response = await fetch(
					`http://localhost:5000/api/posts/${postDetails.post_id}/comments`
				);
				if (response.ok) {
	...
	const toggleLike = async () => {
		try {
			const response = await fetch(
				`http://localhost:5000/api/posts/${postDetails.post_id}/toggle_like`,
				{
					method: "POST",
					credentials: "include",
				}
